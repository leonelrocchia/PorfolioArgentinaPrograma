...
public class JwtProvider {
    ...
    public boolean validateToken(String token){
        try{
            ...
            logger.error("El token ha sido malformado.");
        } catch (UnsupportedJwtException e){
            logger.error("El token no ha sido soportado.");
        } catch (ExpiredJwtException e){
            logger.error("El token ha expirado.");
        } catch (IllegalArgumentException e){
            logger.error("El token es ilegal, está vacío.");
        } catch (SignatureException e){
            logger.error("El token no tiene una firma válida.");
        }
        return false;
    }
}
